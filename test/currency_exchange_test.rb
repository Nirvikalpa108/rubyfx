...
class CurrencyExchangeTest < Test::Unit::TestCase
  ...
  def test_accept_CSV_file
    csv_file = 'data/eurofxref-hist-90d.csv'
    assert_equal(true, ParseFile.new.type_check(csv_file))
  end
...
=begin
  def test_exception_raised_for_non_JSON_or_CSV_file_types
    assert_raise(FileError.new("Error - file type unaccepted")) do
      ParseFile.type_check('data/eurofxref-hist-90d.xml')
    end
  end
=end
end
